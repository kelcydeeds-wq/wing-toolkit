...
export function formatDeviceList({ inputs, outputs }) {
  ...
  return [
    ...
    '',
    '*** This lists WinMM (MME) devices only -- it will NOT show ASIO ***',
    '*** devices, which is what live audio actually needs now (see    ***',
    '*** docs/DECISIONS.md). To see the real ASIO device name, install ***',
    '*** naudiodon (npm install, needs VS Build Tools) and run:        ***',
    '***   node -e "console.log(require(\'naudiodon\').getDevices())"   ***',
    '*** and look for hostAPIName === "ASIO". This tool is still      ***',
    '*** useful for a first look, but do not trust an MME name here   ***',
    '*** for config.audio.inputDevice/outputDevice.                   ***',
    '',
